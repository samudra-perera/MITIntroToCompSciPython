...
investment_return_montly = .04/12
down_payment = total_cost * .25
num_months = 36
num_years = 3
current_savings = 0
#Looking for the amount to save each month as a %
annual_salary = float(input('Enter your annual salary: '))
monthly_salary = annual_salary / 12
montly_savings = 0

#Number of steps in the bisection search
count = 0

#Low value of the guess and high value
low = 0
high = 10000
#initial guess for semi annual rate
guess = (low + high/2.0)
#The delta
epsilon = 100.0

while abs(down_payment - current_savings) >= epsilon:
    #After every iteration reset the current savings
    current_savings = 0
    #Use a temp variable for the annual salary
    temp_annual_salary = annual_salary
    #Turn the rate into a float
    rate = guess /10000
    for n in range(num_months - 1):
        if n % 6 == 0 and n > 0:
            temp_annual_salary= temp_annual_salary * (1 + semi_annual_raise)
            monthly_salary = annual_salary/ 12
            
        montly_savings = monthly_salary * rate
        current_savings += montly_savings
        current_savings *= (1 + investment_return_montly)
